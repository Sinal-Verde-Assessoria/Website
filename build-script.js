...
async function copyRootIndex() {
    console.log('📋 Copiando index.html do root...');
    
    const rootIndexPath = path.join(__dirname, 'index.html');
    const distIndexPath = path.join(CONFIG.paths.dist, 'index.html');
    
    try {
        // Verifica se existe usando fs.promises
        await fs.access(rootIndexPath);
        await fs.copyFile(rootIndexPath, distIndexPath);
        console.log('✅ index.html (redirecionador) copiado para dist/');
    } catch (error) {
        console.log('⚠️ index.html não encontrado no root - criando redirecionador básico');
...
<head>
    ...
    <script>
        ...
        else if (lang.startsWith('es')) targetLang = 'es';
...
<body>
    <div style="text-align:center; padding:50px; font-family:Arial;">
        ...
        <p>Se não for redirecionado, <a href="/pt/">clique aqui</a></p>
    </div>
</body>
</html>`;
...
class PageBuilder {
  ...
  // Encontre a função buildAllPages() (por volta da linha 850) e SUBSTITUA por esta versão:
...
async buildAllPages() {
    ...
    for (const lang of CONFIG.languages) {
        ...
        // Build root pages - COM VERIFICAÇÃO DETALHADA
        const rootTemplates = [
            'index.html',
            'servicos.html',
            'sobre.html',
        ...
        console.log(`    📝 Construindo páginas principais para ${lang}...`);
        
        for (const page of rootTemplates) {
            try {
                const templatePath = path.join(CONFIG.paths.templates, page);
        ...
        // Build blog pages
        console.log(`    📝 Construindo páginas do blog para ${lang}...`);
        await this.buildBlogPages(lang);
        
        // Build segment pages
        console.log(`    📝 Construindo páginas de segmentos para ${lang}...`);
